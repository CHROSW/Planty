	...
	function ultrapress_post_cat_lists()
	{
		...
		if ('post' === get_post_type()) {
			...
			if ($label == true) {
				...
				$label_content = '<span class="cat-label">' . esc_html_e( 'Posted in', 'ultrapress' ) .':' . ' </span> ';
			}
		...
		if ('post' === get_post_type()) {
			...
			if ($posttags) {
				...
				if ($label == true) {
					...
					$label_class = "meta-label";
					$label_content = '<span class="tag-label">' . esc_html_e( 'Tagged', 'ultrapress' ) .':' . ' </span> ';
				}
...
if (!function_exists('ultrapress_get_by_author')) {
	function ultrapress_get_by_author()
	{
		...
		if ($label == true) {
			...
			$label_content = '<span class="author-by">' . esc_html_e( 'By', 'ultrapress' ) .':' . ' </span> ';
		}
		$byline = sprintf(
			...
			foreach ($meta_explodes as $meta_explode) {
				if ('comments' === $meta_explode) {
					...
					if ($label == true) {
						...
						$label_class = "meta-label";
						$label_content = '<span class="cat-label">' . esc_html_e( 'Comments', 'ultrapress' ) .':' . ' </span> ';
					}
			?>
				...
				<?php
					...
					if ($label == true) {
						...
						$label_class = "meta-label";
						$label_content = '<span class="post-label">' . esc_html_e( 'Posted on', 'ultrapress' ) .':' . ' </span> ';
					}

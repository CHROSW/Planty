...
if (!function_exists('ultrapress_starter_setup')) :
	...
	function ultrapress_starter_setup()
	{
		...
		add_image_size('ultrapress_archive_thumbnail', 460, 290, true); //update from 365, 230
...
function ultrapress_enqueue_scripts()
{
	...
	$is_dev = apply_filters('ultrapress_dev_mode', false);
	if ($is_dev == false) {
		$min = '.min';
	} else {
		$min = '';
	...
	/* Styles */
 	ultrapress_google_fonts_url(); // Load google fonts( cdn or local )
	...
	wp_enqueue_style('ultrapress-main-style', get_template_directory_uri() . '/assets/css/style' . $min . '.css');
	wp_enqueue_style('ultrapress-responsive', get_template_directory_uri() . '/assets/css/responsive' . $min . '.css');

	/* Scripts */
	wp_register_script('slick', get_template_directory_uri() . '/assets/js/slick.min.js', array('jquery'), ULTRAPRESS_VERSION, true);
	wp_register_script('isotop', get_template_directory_uri() . '/assets/js/isotope.pkgd.min.js', array('jquery'), ULTRAPRESS_VERSION, true);

	if (is_archive() || is_home()) {
		wp_enqueue_script('isotop');
	}
	wp_enqueue_script('ultrapress-custom', get_template_directory_uri() . '/assets/js/custom' . $min . '.js', array('jquery'), ULTRAPRESS_VERSION, true);
	if (is_singular() && comments_open() && get_option('thread_comments')) {
		wp_enqueue_script('comment-reply');
	}
}
add_action('wp_enqueue_scripts', 'ultrapress_enqueue_scripts', 11);
/**
